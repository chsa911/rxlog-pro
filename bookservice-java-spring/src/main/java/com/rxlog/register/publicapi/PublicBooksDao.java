...
public class PublicBooksDao {
    ...
    public List<PublicBookRow> search(String author, String title, Bucket bucket, int limit) {
        ...
        sql.append("select coalesce(a.full_name, b.author) as author, ")
                .append(titleExpr)
                .append(" as title, ")
                // Optional enrichment fields (fast path; no extra joins)
                .append("b.purchase_source as purchase_vendor, ")
                .append("b.purchase_url as purchase_link ")
                .append("from books b ")
        ...
        return jdbc.query(
                sql.toString(),
                args.toArray(),
                (rs, i) -> new PublicBookRow(
                        rs.getString("author"),
                        rs.getString("title"),
                        rs.getString("purchase_vendor"),
